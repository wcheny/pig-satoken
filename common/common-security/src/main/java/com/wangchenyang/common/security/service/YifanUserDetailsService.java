package com.wangchenyang.common.security.service;
 ...
 * @date 2021/12/21
 */
public interface YifanUserDetailsService extends UserDetailsService, Ordered {
	 ...
	 * @return UserDetails
	 */
	default UserDetails getUserDetails(@Param("result") R<UserInfo> result) {
		...
		// 构造security用户
		return new YifanUser(user.getUserId(), user.getDeptId(), user.getUsername(),
	...
	/**
	 * 通过用户实体查询
	 * @param yifanUser user
	 * @return
	 */
	default UserDetails loadUserByUser(YifanUser yifanUser) {
		return this.loadUserByUsername(yifanUser.getUsername());
	}
